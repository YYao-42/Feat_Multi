...
def extract_box_info_folder(folder_path, video_dict, args):
    ...
    for video in video_dict.keys():
        ...
        box_info_path = box_info_output + video + '_box_info.pkl'
        if not os.path.exists(box_info_path):
            ...
            box_info, fps, frame_width, frame_height = extract_box_info_video(video_path, net, args, LABELS)
...
def extract_box_info_video(video_path, net, args, LABELS):
    ...
    while frame_count < max_frame:
        ...
        if not grabbed:
            break
        # Detect objects 
        boxes, _, _, _, elap_OS = vputils.object_seg_maskrcnn(frame, net, args, LABELS, detect_label=args["detectlabel"])
        if len(boxes) > 1:
...
if args["overlay"]:
    ...
    for pair in video_pairs:
        ...
        v1_box_path = 'videos/OVERLAY/box_info/' + pair[0][:2] + '_box_info.pkl'
        v2_box_path = 'videos/OVERLAY/box_info/' + pair[1][:2] + '_box_info.pkl'
